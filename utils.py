...
def get_monolayer_mono(monolayer_ats):
    num_lead=0
    lead_height=0
    for i in range(len(monolayer_ats.positions)):
        if monolayer_ats[i].symbol == 'Pb' and monolayer_ats[i].position[2]>0:
            lead_height=monolayer_ats[i].position[2]
            break
                
    monolayer_mono = monolayer_ats[[atom.position[2]<lead_height for atom in monolayer_ats]]
    symbols=monolayer_mono.get_chemical_symbols()
    tags = [1 if symbol in ["C", "N", "H"] else 0 for symbol in symbols]
    monolayer_mono.set_tags(tags)
    monolayer_mono = monolayer_mono[[atom.tag == 0 for atom in monolayer_mono]]
    return monolayer_mono
