...
class OrganicMolecule:
    ...
    def __init__(self, ats, charge):
        ...
        ats.set_chemical_symbols(syms)
        
        self.atoms = deepcopy(ats)
        self.charge = charge
    ...
    def __repr__(self):
        string = f"OrganicMolecule: {self.atoms.get_chemical_formula()}"
        string += f"\n charge: {self.charge}"
...
class InorganicMonolayer:
    ...
    def __init__(self, monolayer_ats):
        ...
        pb_z_coords = [atom.position[2] for atom in self.atoms if atom.symbol == "Pb"]
        if len(set(pb_z_coords)) > 1:
            monolayer_mono = get_monolayer_mono(self.atoms)
            self.fitted_normal, self.two_d_direction = find_inorganic_layer_normal(monolayer_mono)
            self.ps_lattice_constants = np.asarray(get_2d_pseudocubic_lattice_vectors(monolayer_mono, self.two_d_direction, self.fitted_normal))
        else :
            self.fitted_normal, self.two_d_direction = find_inorganic_layer_normal(monolayer_ats)
            self.ps_lattice_constants = np.asarray(get_2d_pseudocubic_lattice_vectors(monolayer_ats, self.two_d_direction, self.fitted_normal))
        ...
        self.lead_positions_zero=self.lead_positions
        
        y=0
        for i in range(0,self.lead_positions_zero.shape[0]):
            if self.lead_positions_zero[y,2] != 0:
                self.lead_positions_zero = np.delete(self.lead_positions_zero,y,axis=0)
                y = y-1
            y = y+1
            
        max_lead_position=max(self.lead_positions[:,2])
        y=0
        for i in range(0,self.lead_positions.shape[0]):
            if self.lead_positions[y,2] != max_lead_position:
                self.lead_positions = np.delete(self.lead_positions,y,axis=0)
                y = y-1
            y = y+1
    ...
    @classmethod
    def from_species_specification(cls,B_site,X_site,num_unit_cell_octahedra,num_layer,A_site=0,A_Rx=0,A_Ry=0,A_Rz=0):
        ...
        if A_Ry:
            A_site.rotate(A_Ry, 'y')
        if A_Rz:
            A_site.rotate(A_Rz, 'z') 
        ...
        height = dist*4.0/(2**0.5)+typical_distances[(B_site, X_site)]*2*(num_layer-1)
        unit_cell_dims = {1:(1,1,num_layer), 2:(1,2,num_layer), 4:(2,2,num_layer)}[num_unit_cell_octahedra]
        cell_options = {
        ...
        for index_a in range(unit_cell_dims[0]):
            for index_b in range(unit_cell_dims[1]):
                for index_c in range(unit_cell_dims[2]):
                    species.append(B_site)
                    positions.append([dist*(index_a + index_b), dist*(index_a - index_b), typical_distances[(B_site, X_site)]*2*index_c])
        for index_a in range(unit_cell_dims[0]):
            for index_b in range(unit_cell_dims[1]):
                species += [X_site]
                positions += [[dist*(index_a + index_b), dist*(index_a - index_b), -dist/2**0.5]]
                for index_c in range(unit_cell_dims[2]):
                    species += [X_site]*3
                    positions += [
                        [dist*(index_a + index_b) + dist/2, dist*(index_a - index_b) + dist/2, typical_distances[(B_site, X_site)]*2*index_c],
                        [dist*(index_a + index_b) + dist/2, dist*(index_a - index_b) - dist/2, typical_distances[(B_site, X_site)]*2*index_c],
        ...
        if num_layer != 1:
            if num_unit_cell_octahedra==1:
                for i in range(0,num_layer-1):
                    A_positions=np.array([0,dist,typical_distances[(B_site, X_site)]*(1+(i)*2)])
                    A=cls.place_atoms_at_location(A_site,A_positions)
                    monolayer = monolayer+A
    
            if num_unit_cell_octahedra==2:
                for i in range(0,num_layer-1):
                    A_positions=np.array([dist,0,typical_distances[(B_site, X_site)]*(1+(i)*2)])
                    A=cls.place_atoms_at_location(A_site,A_positions)
                    monolayer = monolayer+A
                    A_positions=np.array([0,dist,typical_distances[(B_site, X_site)]*(1+(i)*2)])
                    A=cls.place_atoms_at_location(A_site,A_positions)
            ...
            if num_unit_cell_octahedra==4:
                for i in range(0,num_layer-1):
                    A_positions=np.array([0,dist,typical_distances[(B_site, X_site)]*(1+(i)*2)])
                    A=cls.place_atoms_at_location(A_site,A_positions)
                    monolayer = monolayer+A
                    A_positions=np.array([0,dist*3,typical_distances[(B_site, X_site)]*(1+(i)*2)])
                    A=cls.place_atoms_at_location(A_site,A_positions)
                    monolayer = monolayer+A
                    A_positions=np.array([dist,dist*2,typical_distances[(B_site, X_site)]*(1+(i)*2)])
                    A=cls.place_atoms_at_location(A_site,A_positions)
                    monolayer = monolayer+A
                    A_positions=np.array([-dist,dist*2,typical_distances[(B_site, X_site)]*(1+(i)*2)])
                    A=cls.place_atoms_at_location(A_site,A_positions)
    ...
    @classmethod    
    def place_atoms_at_location(cls,atoms, r):
        """
        Purpose:
        ...
        mod_atoms = cls.com_to_origin(mod_atoms)
        mod_atoms = cls.translate(mod_atoms, r)

        return(mod_atoms)
        
    @classmethod
    def com_to_origin(cls,atoms):
        """
    ...
    def get_bonding_points(self, normal_displacement=3.5):
        ...
        for i in range(self.lead_positions.shape[0]):
            ...
            bonding_points_south.append(self.lead_positions_zero[i] - self.fitted_normal * normal_displacement + 0.5*(ps1+ps2))
...
class PerovskiteBuilder:
    ...
    def generate_homogeneous_perovskite_samples(
        ...
        # mark A-position atoms to avoid checking
        symbols=inorganic_layer.atoms.get_chemical_symbols()
        tags = [1 if symbol in ["C", "N", "H"] else 0 for symbol in symbols]
        inorganic_layer.atoms.set_tags(tags)
        ...
        for i in range(max_num_attempts):
            ...
            for inner_counter in range(max_attempts_per_symmetry):
                ...
                structure_c = structure_c[[atom.tag == 0 for atom in structure_c]]
                if check_molecule_intersection(structure_c, num_molecules) and check_mol_to_inorganic_intersections(structure_c):
